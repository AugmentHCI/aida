# Guo 2021 model - ATKT - Link: https://github.com/xiaopengguo/ATKT
# coding: utf-8
import torch
...
class KT_backbone(nn.Module):
    ...
    def forward_final_skill_pred(self, skill, answer, interaction_len, perturbation=None):
        # translate to own embedding
        skill_embedding = self.skill_emb(skill)
        answer_embedding = self.answer_emb(answer)
        ...
        print(res.size())
        print(res[:, interaction_len-2, :])
        print(res[:, interaction_len-1, :])

        final_state = res[:, interaction_len-1, :]
        #print(res)

        return final_state, skill_answer_embedding1